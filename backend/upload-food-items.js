...
if (!CONVEX_URL) {
  ...
  console.error("Please create a .env file with CONVEX_URL=your-convex-url");
  process.exit(1);
}
...
async function uploadFoodItems() {
  try {
    ...
    // Upload to Convex
    const result = await client.mutation("foods:seedFoodMenu", {
      foods: foodItems,
    });
    ...
    console.error("\n💡 Troubleshooting:");
    console.error("1. Check that your CONVEX_URL is correct in .env");
    console.error("2. Make sure your Convex deployment is running");
    console.error("3. Verify the mutation 'foods:seedFoodMenu' exists in Convex");
    process.exit(1);
  }
}
