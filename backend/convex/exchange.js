...
export const searchItems = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.itemType) {
            items = items.filter((item) => item.item.toLowerCase().includes(args.itemType.toLowerCase()));
        }
        // Filter by owner block
        if (args.ownerBlock) {
            items = items.filter((item) => item.ownerBlock.toLowerCase().includes(args.ownerBlock.toLowerCase()));
        }
        return items.sort((a, b) => b._creationTime - a._creationTime);
    },
});
...
export const getNearbyItems = query({
    ...
    handler: async (ctx, args) => {
        ...
        return items.filter((item) => {
            ...
            const userBlockLetter = args.userBlock?.match(/Blk ([A-Z])/)?.[1] || args.userBlock?.[0];
            if (!itemBlockLetter || !userBlockLetter)
                return true;
            const distance = Math.abs(itemBlockLetter.charCodeAt(0) - userBlockLetter.charCodeAt(0));
