...
export const searchByName = query({
    ...
    handler: async (ctx, args) => {
        ...
        return allFoods.filter((food) => food.dish.toLowerCase().includes(args.searchTerm.toLowerCase()));
    },
});
...
export const searchByDiet = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.diet && args.diet.length > 0) {
            foods = foods.filter((food) => args.diet.some((d) => food.diet.includes(d)));
        }
        // Filter by tags
        if (args.tags && args.tags.length > 0) {
            foods = foods.filter((food) => args.tags.some((t) => food.tags.includes(t)));
        }
        // Exclude allergens
        if (args.excludeAllergens && args.excludeAllergens.length > 0) {
            foods = foods.filter((food) => !args.excludeAllergens.some((a) => food.allergens.includes(a)));
        }
        // Filter by max price
        if (args.maxPrice) {
